...
\title{Asymptotic constants and finite exclusion tests for the\\
number of alkane isomers:\\
\large computational notes on the sequence A000602}
\author{}
\date{June 12, 2026}
...
$a(n)$ is known. We study several standard mechanisms that often lead to exact
formulas for counting sequences, and give finite exact-arithmetic tests around
them. Specifically, we prove by rigorous modular
...
with period $\le 200$ for $2\le m\le 7$, giving finite evidence against small
digit-automatic descriptions.
We then prove that the \emph{labeled} analogue of the problem admits an exact
two-index finite-sum closed form, emphasizing that the unlabeled quotient is the
main source of difficulty: Burnside's lemma expresses $a(n)$ as a sum over the
partitions of $n$, an index set of unbounded dimension. Finally we show
structurally why the best-known mechanism for collapsing such sums to finite
exact formulas --- Rademacher's modular circle method --- has no evident
analogue here: the generating function is governed by a mixed
$(2,3)$-Mahler-type functional system whose singularities cascade toward a
(conjecturally natural) boundary on the unit circle without any known comparable
transformation theory. Together these results give explicit finite constraints
and a structural picture that any future closed form would have to address.
\end{abstract}
...
validated exact-arithmetic computations, that several standard routes to such a
formula fail in substantial finite ranges, to quantify those ranges, and to
record the structural source of difficulty suggested by the generating
functions. All computations are
...
--- will reappear in \S\ref{sec:structure} as the kind of structure not known
for $a(n)$.
\end{remark}
...
Section~\ref{sec:structure} assembles a structural picture: the Burnside sum
...
absence of an evident Rademacher mechanism. Section~\ref{sec:necessary} states
the resulting constraints on possible solutions of Question~\ref{q:A}.
...
matrices is verified in \S\ref{sec:data}. Implementation risk is controlled by
...
\item The Newton-computed radius of convergence $\rho$
(Proposition~\ref{prop:otter}) agrees with OEIS entry A261340 (decimal
expansion of $\rho$, $97$ digits, V.~Kotesovec 2015) digit-for-digit through
all $42$ digits computed here.
\end{enumerate}
...
The certified staircase $(r+1)(d+1)\lesssim 600$ is large relative to many
familiar examples: the minimal recurrences of numerous classical closed-form
sequences (central binomials, Catalan, Motzkin, Ap\'ery, Domb, families of
lattice-path and constant-term sequences) have $(r+1)(d+1)\le 40$. Thus any
hypergeometric formula for $a(n)$, if it exists within this framework, would
have to lie well outside the tested range, in a regime considerably larger than
...
This rules out small tested versions of a standard route to exact formulas for
the \emph{residues} of $a(n)$, let alone $a(n)$ itself; it is also consistent
with the expectation that $a(n)$ is not one of the rare ``Mahler-automatic''
coincidences in which a $(2,3)$-mixed system degenerates to a digit rule. Such
degenerations are strongly constrained by the rigidity theory of Mahler
equations (Adamczewski--Bell \cite{AdamczewskiBell2017}, Sch\"afke--Singer
\cite{SchafkeSinger2019}), and the low-complexity rational/algebraic
possibilities are excluded here by Theorem~\ref{thm:alg}.
...
dimension growing polynomially. The combinatorial message of
Theorems~\ref{thm:holo}--\ref{thm:autom} is that no such collapse is found in
the tested holonomic, algebraic, or automatic ranges. (For trees,
$\fix(\lambda)$ is supported on special $\lambda$ and is computed in practice
through \eqref{eq:T}--\eqref{eq:A}; the partition sum is a faithful image of the
substitutions $x\mapsto x^m$ in \eqref{eq:T}--\eqref{eq:A}.)
...
\emph{every} rational point of the boundary. Thus a ``natural boundary'' per se
is not necessarily an obstruction to Question~\ref{q:A}; the apparent absence of
a transformation group is the more relevant difficulty.
\end{remark}
...
the reason the dominant singularity alone does not reveal a closed-form
mechanism --- the local behavior is generic square-root, indistinguishable from
that of any other simple variety of trees, with all arithmetic specificity pushed
into the constants $\rho,a,b,C,c_1,\dots$, none of which is known to be
expressible in classical constants.
\end{proof}
...
$\rho$ itself, Proposition~\ref{prop:otter}, is the structurally forced one),
the singularities of $T$, hence of $A$, accumulate densely on $|x|=1$:
...
replaced by ``everywhere''. The certificates of \S\ref{sec:holo} may be viewed
as an unconditional finite shadow of this conditional statement.
\end{corollary}

\subsubsection{No evident transformation theory}
...
\cite{AdamczewskiBell2017,SchafkeSinger2019} --- suggest that a
$\{2,3\}$-mixed situation should not be expected to carry a common structure
except in special cases. This contrasts with the modular situation of
Remark~\ref{rem:rademacher}: $\eta$'s boundary singularities are all
$\mathrm{SL}_2(\ZZ)$-equivalent, while the cascade points
$\rho^{1/(2^a3^b)}\zeta$ of Proposition~\ref{prop:cascade} are not known to be
organized by a comparable group action. A Rademacher-type exact formula for
$a(n)$ would require an exact local expansion at every cascade point \emph{plus}
a summable global organization of those expansions; we know of no mathematical
technology supplying either ingredient in this setting.

%======================================================================
\section{Constraints on possible solutions of (A)}\label{sec:necessary}
%======================================================================
...
$n_0\le 25$, say) would have to satisfy the following constraints.
\begin{enumerate}
...
\item \textbf{Would need a Burnside collapse.} $F$ would have to reproduce
...
uses a modular transformation group; no comparable group is currently known for
the alkane system or for its pure-Mahler shadow
(\S\ref{sec:structure}).
\item \textbf{Exponential precision.} By Proposition~\ref{prop:rounding}, $F$
would have to beat the truncated asymptotic scale \eqref{eq:asym} by a factor
$\alpha^{-n}n^{5/2}$: it would have to encode the constants
$\rho, C, c_1, c_2,\dots$ to unbounded accuracy, i.e.\ contain the full
asymptotic information of the singular expansion, not merely any fixed finite
part of it.
\end{enumerate}
\end{theorem}
...
items (3) and (5) constrain these meaningfully; (ii) a future
``mixed-Mahler circle method'' supplying a currently absent transformation
theory;
(iii) larger certified perimeters (the method of \S\ref{sec:method} scales to
thousands of terms with FFT-based convolution and block elimination). One
possible deeper route to Question~\ref{q:A} would be an exact asymptotic theory
for mixed Mahler systems; even a first nontrivial instance of such a theory
